...
func (f *FluentdConfigDS) GetFluentdConfig(namespace string) (string, error) {
	...
	sortedFcs := make([]string, 0, len(fluentdConfigs))
	for _, fc := range fluentdConfigs {
		fcByName[fc.Name] = fc
		sortedFcs = append(sortedFcs, fc.Name)
	}
	sort.Strings(sortedFcs)

	// Extract fluentd
	configData := make([]string, 0, len(sortedFcs))
	for _, name := range sortedFcs {
		fd := fcByName[name]
