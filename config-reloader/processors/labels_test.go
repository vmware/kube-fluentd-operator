...
func TestLabelsParseOk(t *testing.T) {
	inputs := map[string]map[string]string{
		"**.hello.world":                nil,
		"$labels(a=b,,,)":               map[string]string{"a": "b"},
		"$labels(a=1, b=2)":             map[string]string{"a": "1", "b": "2"},
		"$labels(x=y,b=1)":              map[string]string{"b": "1", "x": "y"},
		"$labels(x=1, b = 1)":           map[string]string{"b": "1", "x": "1"},
		"$labels(x=1, a=)":              map[string]string{"a": "", "x": "1"},
		"$labels(x=1, _container=main)": map[string]string{"_container": "main", "x": "1"},
	}
